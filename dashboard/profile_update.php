...
if (isset($profile_img_update)) {
    ...
    // Use preg_match to validate the file extension
    if (preg_match("/\.(jpg|jpeg|png|gif)$/", $image_name, $matches)) {
        ...
        if (move_uploaded_file($image_temp_name, $path)) {
            ...
            $_SESSION['profile_img_error'] = "Error uploading the profile image.";
            header("location: profile.php");
        }
    } else {
        $_SESSION['profile_img_error'] = "Invalid file extension. Allowed extensions: jpg, jpeg, png, gif";
        header("location: profile.php");
    }
}
